...
Attached report shows how Matt used the data

Baseline Assignment:
1) Generate the network graph of the board of directors interlocks in a nation

Create the bipartite projections and choose one of the networks (firm-firm vs. person-person)


2) Use the skills from the previous week to analyze and assess the structure of the ties
...
Dir2020_no_na <- read.csv("Dir2020raw.csv")
Firm_LU <- read.csv("Firm_LU.csv")
People_LU <- read.csv("People_LU.csv")

Dir2020.net_na_fix <- graph_from_data_frame(Dir2020_no_na, directed = FALSE) # some companies are missing Director ID numbers so it replaces those with NA

V(Dir2020.net_na_fix)$type <- V(Dir2020.net_na_fix)$name %in% Dir2020_no_na$BvD # this however makes bipartite_projection function below work

table(V(Dir2020.net_na_fix)$type)
is_bipartite(Dir2020.net_na_fix)

## take the bipartite projection of the network -- eg firms linked to one another through shared directors
Dir2020.list_na_fix <- bipartite_projection(Dir2020.net_na_fix, type=(Dir2020.net_na_fix)$type, multiplicity = TRUE)
Dir2020.bipar.1_na_fix <- Dir2020.list_na_fix$proj1
Dir2020.bipar.2_na_fix <- Dir2020.list_na_fix$proj2

Directors.2020.dir.net <-Dir2020.bipar.1_na_fix
Firms.2020.dir.net <-Dir2020.bipar.2_na_fix

# Set a vertex attribute from node list
Directors.2020.dir.net<-set_vertex_attr(Directors.2020.dir.net,
                                        name="country",
                                        index = V(Directors.2020.dir.net),
                                        value=People_LU$ISO)
